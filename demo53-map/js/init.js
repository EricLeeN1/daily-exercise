...
$(function () {
    ...
    $("#list .pNode").click(function () {
        ...
        if ($("#list li:eq(" + index + ")").is(":hidden")) {
            $("#list .pNode").css({ "background-color": "#1f6b75" }); //非焦点一级菜单项背景颜色
            $(".leftsidebar_box dt").css({ "background-color": "#339999" }); //非焦点二级菜单项背景颜色
            $("#list .sNode").hide(); //隐藏二级菜单项
            $(".leftsidebar_box dd").hide(); //隐藏三级菜单项
...
/**系统初始默认页面源码显示 **/
$(function () {
    setCore('OSM', 'ThirdPartyMapDisplay','ThirdPartyMapDisplay/E02OSM_help.htm'); //显示默认页面的源码
});
...
function setCore(name, catalog, interFaceName) {
    ...
    if (name == "igsFunDemo" && catalog == "igsFunctionDemo" && interFaceName == "") {
        $("#code_wrapper").hide();
        ...
        $('#container_iframe').attr("src", htmlUrl); //设置右侧容器的页面地址   
         ...
         if ($("#code_wrapper").css('display') == 'none') {
            $("#code_wrapper").show();
