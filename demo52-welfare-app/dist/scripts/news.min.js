"use strict";

function _defineProperty(e, t, n) {
    return t in e ? Object.defineProperty(e, t, {
        value: n,
        enumerable: !0,
        configurable: !0,
        writable: !0
    }) : e[t] = n, e
}
$(function () {
    var e;
    jQuery.support.cors = !0;
    var t, n;
    window.Base;
    _defineProperty(e = {
        site: "https://www.doubaner.top",
        language: (navigator.browserLanguage || navigator.language).toLowerCase(),
        versions: (t = navigator.userAgent, n = t.toLowerCase(), navigator.appVersion, {
            wechat: "micromessenger" == n.match(/MicroMessenger/i),
            qq: "qq" == n.match(/QQ/i),
            weibo: "weibo" == n.match(/WeiBo/i),
            trident: -1 < t.indexOf("Trident"),
            presto: -1 < t.indexOf("Presto"),
            webKit: -1 < t.indexOf("AppleWebKit"),
            gecko: -1 < t.indexOf("Gecko") && -1 == t.indexOf("KHTML"),
            mobile: !!t.match(/AppleWebKit.*Mobile.*/),
            ios: !!t.match(/\(i[^;]+;( U;)? CPU.+Mac OS X/),
            android: -1 < t.indexOf("Android") || -1 < t.indexOf("Linux"),
            iPhone: -1 < t.indexOf("iPhone"),
            iPad: -1 < t.indexOf("iPad"),
            webApp: -1 == t.indexOf("Safari")
        }),
        fontSize: function () {
            var e = 0 < arguments.length && void 0 !== arguments[0] ? arguments[0] : window,
                t = e.document,
                n = t.documentElement,
                i = "orientationchange" in e ? "orientationchange" : "resize",
                a = function () {
                    var e = n.clientWidth || 375;
                    750 < e && (e = 750), n.style.fontSize = e / 7.5 + "px"
                };
            window.getComputedStyle(n, null).getPropertyValue("font-size") <= "20px" && a(), t.addEventListener && (e.addEventListener(i, a, !1), t.addEventListener("DOMContentLoaded", a, !1))
        },
        setSeo: function () {
            this.getAjax("/seo", {}, function (e) {
                console.log(e), 200 == e.status ? ($("title").text(e.data.title), $("meta[name='keywords']").attr("content", e.data.keywords), $("meta[name='description']").attr("content", e.data.description)) : alert(e.message)
            })
        },
        showNav: function () {
            $(".common-header ol").toggle(), $("body").toggleClass("header-active")
        },
        getInfos: function () {
            var a = this,
                e = a.getQueryString("id");
            a.postAjax("/new", {
                id: e
            }, function (e) {
                if (console.log(e), 200 == e.status) {
                    var t = $("#news-body>main>article"),
                        n = [],
                        i = e.data.str_content;
                    t.attr("data-id", e.data.cat_id), t.children("h3").html(e.data.str_title), t.children(".icon").attr("src", a.site + e.data.str_thumb).show(), t.children(".time").html(e.data.news_time), i.replace(/<img [^>]*src=['"]([^'"]+)[^>]*>/gi, function (e, t) {
                        n.push(t)
                    }), n && n.forEach(function (e) {
                        var t = a.site + e;
                        i.replace(e, t)
                    }), t.append(i);
                    var data = e.data;
                    if (data.PrevArticleId) {
                        $("#prev").attr('href',`./news.html?id=${data.PrevArticleId}`);
                        $("#prev P").text(data.PrevArticleTitle);
                    } else {
                        $("#prev").attr('href',`javascript:;`);
                        $("#prev P").text('已经是最新一篇了');
                    }
                    if (data.NextArticleId) {
                        $("#next").attr('href',`./news.html?id=${data.NextArticleId}`);
                        $("#next P").text(data.NextArticleTitle);
                    } else {
                        $("#next").attr('href',`javascript:;`);
                        $("#next P").text('已经是最新一篇了');
                    }
                } else alert(e.message)
            })
        },
        getQueryString: function (e) {
            var t = new RegExp("(^|&)" + e + "=([^&]*)(&|$)", "i"),
                n = window.location.search.substr(1).match(t),
                i = "";
            return null != n && (i = n[2]), (n = t = null) == i || "" == i || "undefined" == i ? "" : i
        }
    }, "setSeo", function () {
        this.getAjax("/seo", {}, function (e) {
            console.log(e), 200 == e.status ? ($("title").text(e.data.title), $("meta[name='keywords']").attr("content", e.data.keywords), $("meta[name='description']").attr("content", e.data.description)) : alert(e.message)
        })
    }), _defineProperty(e, "getAjax", function (e, t, n, i) {
        $.ajax({
            type: "GET",
            url: this.site + e,
            data: t,
            dataType: "JSON",
            success: n,
            fail: i
        })
    }), _defineProperty(e, "postAjax", function (e, t, n, i) {
        $.ajax({
            type: "POST",
            url: this.site + e,
            data: t,
            dataType: "json",
            success: n,
            fail: i
        })
    }), _defineProperty(e, "init", function () {
        console.log("111");
        var e = this;
        e.getInfos(), e.fontSize(), e.setSeo(), $(".common-header ul").on("click", function () {
            e.showNav()
        })
    }), e.init()
});
//# sourceMappingURL=news.js.map